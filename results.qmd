...
str(names(payroll_data))
```


```{r}
payroll_data_fire_police <- payroll_data_fire_police |>
  mutate(start_year = lubridate::year(agency_start_date)) |> 
  filter(start_year > 2014)

# Create the bar chart
ggplot(payroll_data_fire_police, aes(x = start_year, fill = agency_name)) +
  geom_bar() +
  labs(
...
payroll_data_fire_police <- 
  ...
  mutate(agency_start_date_clean = agency_start_date)
