...
ggplot(summarized_data) +
  ...
  geom_hline(
    ...
    aes(yintercept = max_records, color = "Highest value in 10 years"),
    # inherit.aes = FALSE,
    linetype = "dashed"
  ) +
  geom_text(
  ...
  scale_color_manual(
    values = c("Highest value in 10 years" = "black") 
  ) +
  theme_minimal() +
  theme(
...
kable(payroll_data_fire_police_mosaic_table, align = "lccccc") |>
...
payroll_data_fire_police_isolation <-
  payroll_data_fire_police |> 
  mutate(
  ...
  mutate(
    years_at_agency = 2024 - agency_start_date_year, 
...
payroll_data_fire_police |> 
  ...
  mutate(
    label = ifelse(row_number() <= 10, title_description, NA),
    is_top10 = factor(row_number() <= 10, levels = c(TRUE, FALSE), labels = c("Top 10", "Other Job Titles"))
  ) |> 
  ggplot() +
  geom_point(
    mapping = aes(
  ...
  scale_fill_distiller(
    ...
    name = "Salary Paid Difference YoY",
    guide = "none" 
  ) +
  scale_color_manual(
    values = c("Top 10" = "orange", "Other Job Titles" = 'deepskyblue3'),
    guide = "none"
  ) +
  labs(
...
payroll_data_fire_police_experience <-
  payroll_data_fire_police |> 
...
essay_grades_frequency_lode <- essay_grades_frequency_lode |> 
  mutate(
    stratum_width = ifelse(stratum == "Total", 2, 1)
  )
...
payroll_data_fire_police_breakdown <-
  payroll_data_fire_police |> 
  filter(fiscal_year == 2024) |>
  mutate(
    flag_regular = ifelse(regular_gross_paid == 0, "No", "Yes"),
...
payroll_data_fire_police_breakdown_250 <-
payroll_data_fire_police_breakdown |> 
  filter(total_paid_group == "250k and above") |> 
...
payroll_data_fire_police_breakdown_250_selection <-
  payroll_data_fire_police_breakdown_250 |> 
...
ggparcoord(data = payroll_data_fire_police_breakdown_250_selection,
           columns = 3:6,
           alphaLines = 0.1,
...
parcoords(
  payroll_data_fire_police_breakdown_250[, c(
    # "agency_name_order", 
    "order_sequence",
    "agency_name_order", 
    # "title_description_short",
...
payroll_data_fire_police |>
  ...
  scale_x_continuous(
    ...
    name = "Salary (dots)",
    breaks = c(-250000, -187500, -125000, -62500, 0, 100000,200000), 
